...
val success =spark.sql("Select count(case y when 'yes' then 1 end)/count(*)*100 as success_rate from bankdata") 
success.writeStream.outputMode("update").option("truncate",false).format("console").start().awaitTermination(10)

print("Give marketing failuer rate:")
val fail=spark.sql("Select count(case y when 'no' then 1 end)/count(*)*100 as failure_rate from bankdata") 
...
val ageMatters = spark.sql(q5) ageMatters.writeStream.outputMode("update").option("truncate", false).format("console").start().awaitTermination(10)
...
val status = spark.sql(q6) status.writeStream.outputMode("update").option("truncate", false).format("console").start().awaitTermination(10)
